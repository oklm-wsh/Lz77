...
struct
  ...
  let rsub t off len =
    ...
    assert (off + len < available_to_read t);

    let buff = Scalar.create len in

    let pre = t.wpos - off in
    let pos = if pre < 0 then t.size + (* - *) pre else pre in
    let pre = t.size - pos in
    let extra = len - pre in

    if extra > 0 then begin
      Scalar.blit t.buffer pos buff 0 pre;
      Scalar.blit t.buffer 0 buff pre extra;
    end else
      Scalar.blit t.buffer pos buff 0 len;
